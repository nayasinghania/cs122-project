...
This Weather Analysis App generates a historical graph of weather data. Users will be able to enter a US city of their choosing to get a time series plot of low and high temperatures throughout 3 time frames: 7 days, 30 days, and 1 year. Users will not only be able to see how temperatures rise and fall daily but can also view best fit trend lines of the data, and see averages for low and high historical temperatures. They can also zoom in and out of the graph and save the plot as a PNG file onto their device. Additionally, users can also view the current temperature and overcast of their selected city, which will be displayed in a separate popup window. Our interactive widgets are as follows: Entry box to enter a city, submit button to submit the box entry, zoom in to plot button, zoom out of plot button, save plot as PNG button, show current weather button (opens a separate popup window), and a dropdown to see a plot for a 7 day, 30 day, or 1 year period. The data is pulled from a free weather API and stored as CSV for time series data and JSON for current weather data. NumPy is used for data analysis and displaying average highs and lows for a selected time period. 
